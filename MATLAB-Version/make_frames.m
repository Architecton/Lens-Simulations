function [] = make_frames()
    ...
    for frame = 1:length(vals)
        ...
        % initialize lens
        lens.equation = @(x, y, z) (x - vals(frame)).^2 + y.^2 + (z - 1).^2 - 0.64;
        ...
        multiple_entries = 0;
        F_trans = apply_lensing_all(F, lens, 1, multiple_entries);
        ...
        plot_ellipsoid([vals(frame); 0; 1], 0.8, [1; 1; 1]);
        axis equal
		% axis();
        view(-60, 45);
		axis([0 15 -4 4 0 6])
		...
		view(0, 90);
		frame_name = sprintf('b/%d.jpg', frame);
		saveas(gcf, frame_name);
